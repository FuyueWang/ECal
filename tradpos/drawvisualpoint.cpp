...
void drawvisualpoint(){
  ...
  for(Int_t scani=0;scani<Nbofscan;scani++){
	 ...
	 TFile* inrootfile=new TFile(rootdatadir+"rootdata/lwaveform"+TString::Format("%d",scani+1)+"cutLED3by4.root");
	 ...
	 // TCanvas *c1 = new TCanvas("c1","Plot",0,0,1300,800);
	 // c1->Divide(8,4);
	 // gStyle->SetOptStat(0);	
	 // for(Int_t k=0;k<Nboffile-1;k++){
	 // 	c1->cd(k+1);
	 // 	integralhist[scani][k]->Draw("colz");
	 // }
	 // c1->SaveAs(plotdir+"integral2Dscan"+TString::Format("%d",scani+1)+".pdf");
  }
